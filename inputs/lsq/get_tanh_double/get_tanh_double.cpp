...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    if (argc > 3 && DATA_DISTR == data_distribution::PERCENTAGE_WAIT) {
  ...
  try {
    ...
    double sum_fpga = std::accumulate(A.begin(), A.end(), 0.0);
    double sum_cpu = std::accumulate(A_cpu.begin(), A_cpu.end(), 0.0);
    if (almost_equal(sum_fpga, sum_cpu)) {
      std::cout << "Passed\n";
    }
    else {
      std::cout << "Failed" << "\nsum(A_fpga) = " << sum_fpga
